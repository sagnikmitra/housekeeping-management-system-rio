...
class Complaint(models.Model):
    ...
    feedback = models.TextField(max_length=500, blank=False, validators=[
                                validateSubjectFeedback])
    lodge_date = models.DateField(blank=False, default=datetime.today().date())
    ld = datetime.now()
    addressing_date = models.DateField(blank=False, default=datetime.today(
    ).date()+timedelta(days=1), validators=[validateAddressingDate(ld)])
